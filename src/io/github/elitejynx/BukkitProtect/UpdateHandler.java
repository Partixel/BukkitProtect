	...
	private class UpdateRunnable implements Runnable {
		...
		public void run() {
			if (UpdateHandler.this.url != null) {
				...
				if (UpdateHandler.this.read()) {
					if (UpdateHandler.this
							.versionCheck(UpdateHandler.this.versionName)) {
						if ((UpdateHandler.this.versionLink != null)
								&& (UpdateHandler.this.type != UpdateType.NO_DOWNLOAD)) {
							String name = UpdateHandler.this.file.getName();
							// If it's a zip file, it shouldn't be downloaded as
							// the plugin's name
							if (UpdateHandler.this.versionLink.endsWith(".zip")) {
								final String[] split = UpdateHandler.this.versionLink
										.split("/");
								name = split[split.length - 1];
							}
							UpdateHandler.this.saveFile(new File(
									UpdateHandler.this.plugin.getDataFolder()
											.getParent(),
									UpdateHandler.this.updateFolder), name,
									UpdateHandler.this.versionLink);
						} else {
							UpdateHandler.this.result = UpdateResult.UPDATE_AVAILABLE;
						}
					}
				}
			}
